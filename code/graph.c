...
graph_t *new_graph(int nnode, int nedge) {
    ...
    // Allocation for the list of hubs
    g->hub_count = 0;
    g->hubs = calloc(nnode / 6, sizeof(int));
    ok = ok && g->hubs != NULL;
...
graph_t *read_graph(FILE *infile) {
	...
	while (nid < hid) {
	    ...
	    g->neighbor_start[nid] = eid;
            // register hubs when the number of neighbors are large!
            if(eid - prev_neighbor_start > 6){
              g->hubs[hub_count++] = g->neighbor[prev_neighbor_start];
            }
            prev_neighbor_start = eid; 
	    // Self edge
	    g->neighbor[eid++] = nid;
	}
	g->neighbor[eid++] = tid;
